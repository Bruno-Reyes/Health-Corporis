...
io.on("connection", async(socket) => {
    ...
    socket.on("send-message-from-admin", async(data, cb) => {
        ...
        if (name in users) {
            if (msg != null) {
                if (validator.isAlphanumeric(msg)) {
                    if (validator.isLength(msg, { min: 5, max: 100 })) {
                        ...
                        cb("Error! El mensaje debe de tener un largo de entre 5 a 100");
                    }
                } else {
                    cb("Error! Solo se permiten letras y numeros");
                }
            } else {
                cb("Error! Mensaje vacio");
            }
        } else {
