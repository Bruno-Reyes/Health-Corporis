function val() {
   ...
   if (validator.isEmpty(name, { ignore_whitespace: true }) == true) {
      ...
      return false;
   } else if (validator.isLength(grup, { min: 1, max: 1 }) == false) {
      toastr.warning('Elige una opcion del tipo de grupo alimenticio valida.');
      return false;
   } else if (validator.isLength(measure, { min: 1, max: 1 }) == false) {
      toastr.warning('Elige una opcion del tipo de unidad de medida valida.');
      return false;
   }
}