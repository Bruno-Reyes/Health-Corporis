...
const registrarSeguimiento = () => {
    ...
    if (validator.isEmpty(peso, { ignore_whitespace: true }) === true || validator.isEmpty(estatura, { ignore_whitespace: true }) === true) {
    ...
    } else if (validator.isNumeric(peso, { ignore_whitespace: true }) === false) {
        f = false
        ...
        M.toast({ html: message })
    } else if (parseInt(peso) < 20 || parseInt(peso) > 250) {
        f = false
        message = 'El campo peso no puede ser menos de 20 Kg ó mayor a 250 Kg, intentalo de nuevo'
        M.toast({ html: message })
        ...
        M.toast({ html: message })
    } else if (parseInt(estatura) < 90 || parseInt(estatura) > 220) {
        f = false
        message = 'El campo estatura no puede ser menos de 90 cm ó mayor a 220 cm, intentalo de nuevo'
        M.toast({ html: message })
    }
